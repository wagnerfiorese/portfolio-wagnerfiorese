...
const ProjectCard = ({ title, description, techs, link, image }) => {
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.97 }}
      className="bg-white dark:bg-zinc-800 rounded-xl shadow-md p-4 transition-all w-full max-w-xs mx-auto"
    >
      {image && (
      ...
      <ul className="flex flex-wrap justify-center gap-3 mb-4 list-none">
        {techs.map((tech, index) => (
          <li
            key={index}
            className="text-sm bg-indigo-100 dark:bg-indigo-600 text-indigo-800 dark:text-white px-4 py-2 rounded-full min-w-[100px] text-center"
          >
            {tech}
          </li>
        ))}
